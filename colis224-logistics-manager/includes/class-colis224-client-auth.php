...
class Colis224_Client_Auth {
    ...
    public static function login_client($client_id) {
        ...
        // CORRECTION CRITIQUE: Nettoyer complètement toute session existante AVANT de connecter le nouveau client
        // Cela évite les conflits si un autre client était connecté
        error_log('COLIS224 LOGIN_CLIENT: Nettoyage de toute session existante...');
        self::logout_client();
    ...
    public static function logout_client() {
        error_log('COLIS224 LOGOUT: Début de la déconnexion');
        ...
        if (session_id()) {
            ...
            error_log('COLIS224 LOGOUT: Variables de session supprimées');
        }

        // CORRECTION CRITIQUE: Supprimer le cookie avec TOUS les chemins possibles
        // Parfois le cookie persiste si le chemin ne correspond pas exactement
        $cookie_paths = array(
        ...
        $cookie_paths = array_unique($cookie_paths);
        ...
        foreach ($cookie_paths as $path) {
            if (isset($_COOKIE['colis224_client_session'])) {
                ...
                error_log('COLIS224 LOGOUT: Cookie supprimé pour path=' . $path);
            }
        }

        // Forcer la suppression du cookie côté PHP
        unset($_COOKIE['colis224_client_session']);

        error_log('COLIS224 LOGOUT: Déconnexion terminée');
    }
    ...
    public function destroy_client_session() {
        error_log('COLIS224 DESTROY_SESSION: Début de la destruction complète');
        ...
        if (session_id()) {
            ...
            foreach ($_SESSION as $key => $value) {
                if (strpos($key, 'colis224_') === 0) {
                    unset($_SESSION[$key]);
                    error_log('COLIS224 DESTROY_SESSION: Variable supprimée - ' . $key);
                }
            }
        }
        ...
        foreach ($cookie_paths as $path) {
            if (isset($_COOKIE['colis224_client_session'])) {
                ...
                error_log('COLIS224 DESTROY_SESSION: Cookie supprimé pour path=' . $path);
            }
        }
        ...
        error_log('COLIS224 DESTROY_SESSION: Destruction complète terminée');
    }
    ...
    private static function restore_session_from_cookie() {
        ...
        // CORRECTION CRITIQUE: Vérifier qu'il n'y a pas déjà un autre client en session
        if (isset($_SESSION['colis224_client_id']) && $_SESSION['colis224_client_id'] != $client_id) {
            error_log('COLIS224 RESTORE: CONFLIT - Session existe déjà pour client ID=' . $_SESSION['colis224_client_id'] . ', cookie pour ID=' . $client_id);
            // NE PAS restaurer pour éviter le conflit - garder la session actuelle
            return false;
        }
