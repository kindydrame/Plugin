...
class Colis224_Parcels {
    ...
    private static function display_form($parcel_id = null) {
            ...
            <div class="colis224-card">
                <form method="post" id="colis224-parcel-form" enctype="multipart/form-data">
                    ...
                    <div class="colis224-form-grid">
                        ...
                        <!-- Client avec sélection rapide améliorée -->
                        <div class="colis224-form-group colis224-full-width">
                            ...
                            <!-- Conteneur de sélection rapide -->
                            <div class="colis224-client-selector">
                                <!-- Champ de recherche avec boutons -->
                                <div class="colis224-client-search-wrapper">
                                    ...
                                    <button type="button" id="btn-show-all-clients" class="button colis224-btn-client-list" title="Voir tous les clients">
                                        <span class="dashicons dashicons-list-view"></span>
                                    </button>

                                    <!-- Bouton pour effacer la sélection -->
                                    <button type="button" id="btn-clear-client" class="button colis224-btn-clear-client" title="Effacer la sélection" style="display: none;">
                                        <span class="dashicons dashicons-no-alt"></span>
                                    </button>
                                </div>

                                <!-- Clients récents (affichés par défaut) -->
                                <div id="recent-clients-wrapper" class="colis224-recent-clients">
                                    <span class="colis224-recent-label">Clients récents :</span>
                                    <div id="recent-clients-list" class="colis224-recent-list">
                                ...
                                <div id="selected-client-info" class="colis224-selected-client-info" style="display: none;">
                                    <div class="client-info-content">
                                        <span class="dashicons dashicons-yes-alt"></span>
                                        <span id="selected-client-name"></span>
                                        <span id="selected-client-phone" class="client-phone"></span>
                                    </div>
                                </div>
                            </div>
                        </div>

                        <!-- Modal de sélection de clients -->
                        <div id="colis224-clients-modal" class="colis224-modal" style="display: none;">
                            <div class="colis224-modal-content colis224-modal-large">
                                <div class="colis224-modal-header">
                                    <h3><span class="dashicons dashicons-groups"></span> Sélectionner un Client</h3>
                                    <button type="button" class="colis224-modal-close" id="close-clients-modal">
                                        <span class="dashicons dashicons-no-alt"></span>
                                    </button>
                                </div>
                                <div class="colis224-modal-body">
                                    <!-- Recherche dans le modal -->
                                    <div class="colis224-modal-search">
                                        <input type="text" id="modal-client-search" placeholder="Rechercher par nom, téléphone ou email..." autocomplete="off">
                                        <span class="dashicons dashicons-search"></span>
                                    </div>

                                    <!-- Filtres rapides -->
                                    <div class="colis224-modal-filters">
                                        <button type="button" class="colis224-filter-btn active" data-filter="all">Tous</button>
                                        <button type="button" class="colis224-filter-btn" data-filter="recent">Récents</button>
                                        <button type="button" class="colis224-filter-btn" data-filter="frequent">Fréquents</button>
                                    </div>

                                    <!-- Liste des clients -->
                                    <div id="modal-clients-list" class="colis224-modal-clients-list">
                                        <div class="colis224-loading">
        ...
        jQuery(document).ready(function($) {
            ...
            // === SÉLECTION RAPIDE DE CLIENTS AMÉLIORÉE ===

            // Charger les clients récents au démarrage
            loadRecentClients();

            // Fonction pour charger les clients récents
            function loadRecentClients() {
                $.ajax({
                    ...
                    dataType: 'json',
                    data: {
                        action: 'colis224_get_recent_clients',
                    ...
                    success: function(response) {
                        if (response.success && response.data && response.data.length > 0) {
                            ...
                            response.data.forEach(function(client) {
                                html += '<button type="button" class="colis224-recent-client-btn" ' +
                                        'data-id="' + client.id + '" ' +
                            ...
                            $('#recent-clients-list').html(html);
                        } else {
                            $('#recent-clients-wrapper').hide();
                        }
                    },
                    error: function() {
                        $('#recent-clients-wrapper').hide();
            ...
            // Clic sur un client récent
            $(document).on('click', '.colis224-recent-client-btn', function() {
                var id = $(this).data('id');
            ...
            function selectClient(id, name, phone) {
                $('#client_id').val(id);
                $('#client_search').val(name + (phone ? ' - ' + phone : ''));
                $('#btn-clear-client').show();

                // Afficher info client sélectionné
                $('#selected-client-name').text(name);
                $('#selected-client-phone').text(phone || '');
                $('#selected-client-info').slideDown();

                // Masquer les clients récents
                $('#recent-clients-wrapper').slideUp();

                // Charger les infos de fidélité
                loadLoyaltyInfo(id);

                // Fermer le modal si ouvert
                $('#colis224-clients-modal').fadeOut();
            }
            ...
            $('#btn-clear-client').on('click', function() {
                ...
                $('#client_search').val('');
                $(this).hide();
                $('#selected-client-info').slideUp();
                $('#recent-clients-wrapper').slideDown();
            ...
            // Ouvrir le modal
            $('#btn-show-all-clients').on('click', function() {
                $('#colis224-clients-modal').fadeIn();
                loadAllClients();
                $('#modal-client-search').focus();
            });

            // Fermer le modal
            $('#close-clients-modal, .colis224-modal').on('click', function(e) {
                if (e.target === this) {
                    $('#colis224-clients-modal').fadeOut();
                }
            });
            ...
            $('.colis224-modal-content').on('click', function(e) {
                e.stopPropagation();
            });

            // Charger tous les clients dans le modal
            function loadAllClients(filter, searchTerm) {
                filter = filter || 'all';
                searchTerm = searchTerm || '';

                $('#modal-clients-list').html('<div class="colis224-loading"><span class="dashicons dashicons-update spin"></span> Chargement...</div>');
                ...
                $.ajax({
                    ...
                    dataType: 'json',
                    data: {
                        action: 'colis224_get_all_clients',
                        nonce: colis224Ajax.autocomplete_nonce,
                        filter: filter,
                        search: searchTerm
                    },
                    success: function(response) {
                        if (response.success && response.data && response.data.length > 0) {
                            var html = '<div class="colis224-clients-grid">';
                            response.data.forEach(function(client) {
                                html += '<div class="colis224-client-card" data-id="' + client.id + '" ' +
                                        'data-name="' + escapeHtml(client.name) + '" ' +
                                        'data-phone="' + escapeHtml(client.phone || '') + '">' +
                                        '<div class="client-avatar"><span class="dashicons dashicons-admin-users"></span></div>' +
                                        '<div class="client-details">' +
                                        '<div class="client-name">' + escapeHtml(client.name) + '</div>' +
                                        '<div class="client-phone">' + escapeHtml(client.phone || 'Pas de téléphone') + '</div>' +
                                        (client.company_name ? '<div class="client-company">' + escapeHtml(client.company_name) + '</div>' : '') +
                                        (client.parcels_count ? '<div class="client-stats">' + client.parcels_count + ' colis</div>' : '') +
                                        '</div>' +
                                        '</div>';
                            });
                            html += '</div>';
                            $('#modal-clients-list').html(html);
                        } else {
                            $('#modal-clients-list').html('<div class="colis224-no-results"><span class="dashicons dashicons-warning"></span> Aucun client trouvé</div>');
                        }
                    },
                    error: function() {
                        $('#modal-clients-list').html('<div class="colis224-error">Erreur de chargement</div>');
                    }
                });
            }

            // Clic sur une carte client dans le modal
            $(document).on('click', '.colis224-client-card', function() {
                var id = $(this).data('id');
                var name = $(this).data('name');
                var phone = $(this).data('phone');
                selectClient(id, name, phone);
            ...
            $('#modal-client-search').on('input', function() {
                ...
                var activeFilter = $('.colis224-filter-btn.active').data('filter');

                clearTimeout(searchTimeout);
                searchTimeout = setTimeout(function() {
                    loadAllClients(activeFilter, searchTerm);
                }, 300);
            });

            // Filtres dans le modal
            $('.colis224-filter-btn').on('click', function() {
                $('.colis224-filter-btn').removeClass('active');
                $(this).addClass('active');
                var filter = $(this).data('filter');
                var searchTerm = $('#modal-client-search').val();
                loadAllClients(filter, searchTerm);
            });
            ...
            $('#client_search').autocomplete({
                ...
                select: function(event, ui) {
                    selectClient(ui.item.id, ui.item.value.split(' - ')[0], ui.item.phone);
                    return false;
                },
                ...
                // Afficher les suggestions au focus si le champ est vide
                if ($(this).val() === '' && !$('#client_id').val()) {
                    $(this).autocomplete('search', '');
                }
            });

            // Afficher le bouton clear si un client est déjà sélectionné
            <?php if ($is_edit && $parcel->client_id): ?>
            $('#btn-clear-client').show();
            $('#recent-clients-wrapper').hide();
            $('#selected-client-info').show();
            $('#selected-client-name').text('<?php echo esc_js($client ? $client->name : ''); ?>');
            $('#selected-client-phone').text('<?php echo esc_js($client ? $client->phone : ''); ?>');
            <?php endif; ?>
